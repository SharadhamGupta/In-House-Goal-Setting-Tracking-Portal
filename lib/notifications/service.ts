...
import type { Goal, GoalCycle, Quarter, User } from "@/lib/domain/types";
import { createClient } from "@/lib/supabase/server";
import { getSupabaseConfig } from "@/lib/supabase/config";
import { sendMail } from "./mailer";
import { buildEscalationEmail, buildWorkflowEmail, buildCheckInWindowOpenEmail, type NotificationEventType } from "./templates";
...
const eventSubjects: Record<NotificationEventType, string> = {
  ...
  escalation_alert: "AtomBerg GoalHub Escalation Alert"
};
...
export async function sendCheckInWindowOpenEmail(
  supabase: SupabaseClient,
  quarter: Quarter,
  cycle: GoalCycle,
  employees: User[]
) {
  const ctaHref = appUrl("/reviews");

  let closeDateStr = "";
  if (quarter === "Q1") closeDateStr = new Date(cycle.q1ClosesAt).toDateString();
  else if (quarter === "Q2") closeDateStr = new Date(cycle.q2ClosesAt).toDateString();
  else if (quarter === "Q3") closeDateStr = new Date(cycle.q3ClosesAt).toDateString();
  else if (quarter === "Q4") closeDateStr = new Date(cycle.q4ClosesAt).toDateString();

  for (const employee of employees) {
    const dedupeKey = `checkin_window_open_${quarter}_${cycle.id}_${employee.id}`;

    // Insert in-app notification
    await insertNotification(supabase, {
      recipientId: employee.id,
      actorId: employee.id, // system/self triggered
      eventType: "quarterly_checkin_reminder",
      title: `${quarter} Check-in Window Open`,
      message: `The ${quarter} check-in window is now open. Closes: ${closeDateStr}.`,
      ctaHref,
      dedupeKey,
      metadata: { quarter, cycleId: cycle.id }
    });

    // Send email log and SMTP send
    await sendEmail(supabase, {
      recipientId: employee.id,
      to: employee.email,
      eventType: "quarterly_checkin_reminder",
      subject: `${quarter} Check-in Window is Now Open — AtomBerg GoalHub`,
      dedupeKey,
      html: buildCheckInWindowOpenEmail(employee.name, quarter, closeDateStr, ctaHref)
    });
  }
}
