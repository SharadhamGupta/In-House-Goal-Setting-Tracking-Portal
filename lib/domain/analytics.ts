...
export function buildDashboardAnalytics(
  ...
  const checkInCompletionRate = approvedGoals.length
    ? Math.round((approvedGoals.filter((goal) => {
        const goalAchs = achievements.filter((a) => a.goalId === goal.id);
        return goalAchs.some((a) => a.managerCheckinCompleted === true);
      }).length / approvedGoals.length) * 100)
    : 0;
  const pendingCheckIns = Math.max(approvedGoals.length - approvedGoals.filter((goal) => {
    const goalAchs = achievements.filter((a) => a.goalId === goal.id);
    return goalAchs.some((a) => a.managerCheckinCompleted === true);
  }).length, 0);
  ...
  const quarterlyTrend = quarters.map((quarter) => {
    ...
    const average = quarterUpdates.length
      ...
      : 0;
    const completed = quarterUpdates.filter((achievement) => achievement.managerCheckinCompleted === true).length;
  ...
  const heatmapRows = visibleUsers.map((user) => {
    ...
    const cells = quarters.map((quarter) => {
      ...
      return {
        ...
        progress,
        checkIns: quarterUpdates.length,
        completed: quarterUpdates.filter((achievement) => achievement.managerCheckinCompleted === true).length
      };
    });
    return {
      id: user.id,
  ...
  const achievementActivity = achievements
    ...
    .map<ActivityItem>((achievement) => {
      ...
      return {
        ...
        detail: `${goalTitle}: ${achievement.progressPercent}% progress · ${achievement.status.replace("_", " ")}`,
