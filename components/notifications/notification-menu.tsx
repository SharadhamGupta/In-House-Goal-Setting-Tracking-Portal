...
export function NotificationMenu({ notifications, onMarkRead, role }: Props) {
          ...
          <div className="max-h-96 overflow-y-auto">
            {notifications.length ? (
              notifications.map((notification) => {
                const effectiveCtaHref = role === "admin" ? "/reviews?tab=reviews" : role === "employee" ? "/reviews" : (notification.ctaHref ?? "#");
                const hasCta = role === "admin" || role === "employee" || !!notification.ctaHref;
                return (
                  <a
                    key={notification.id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {hasCta ? <ExternalLink className="mt-0.5 h-3.5 w-3.5 shrink-0 text-muted-foreground" /> : null}
                    </div>
                  </a>
                );
              })
